...
# Call Dialog
w = ask(message='Enter Cell Shape value')
x = ask(message='Enter bare - nuclei value')
y = ask(message='Enter Bland value')
z = ask(message='Enter mitoses value')
print ('Cell Shape:', w)
print ('Bare - Nuclei:', x)
print ('Bland:', y)
print ('Mitoses:', z)
...
z2 = (type(z1))
def new_samples():
  pred = np.array([[w1, x1, y1, z1],
                   ], dtype=np.int32)
  return pred
predictions = list(classifier.predict(input_fn=new_samples))
import easygui
easygui.msgbox("Predicted Result is:"+repr(predictions), title="Result")
print(
